...
async def download_file(client, message):
    ...
    dcode_msg = await client.ask("Enter the download code:", filters=filters.text)
    dcode = dcode_msg.text

    resolution_msg = await client.ask("Enter the resolution (e.g., 1080p, 720p, 480p, 360p):", filters=filters.text)
    resolution = resolution_msg.text

    format_msg = await client.ask("Enter the file format (e.g., mp4, mkv (if you need softcoded subtitles)):", filters=filters.text)
    file_format = format_msg.text

    filename_msg = await client.ask("Enter the filename:", filters=filters.text)
